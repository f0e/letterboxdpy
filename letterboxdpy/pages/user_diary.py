...
class UserDiary:
    ...
    def get_diary(
        ...
        return extract_user_diary(
            self.username, year, month, day, page, fetch_runtime, max_workers
        )
    ...
    def get_year(
        ...
        return extract_user_diary(
            self.username, year, fetch_runtime=fetch_runtime, max_workers=max_workers
        )
...
def _get_runtime(slug: str) -> int | None:
...
def _fetch_missing_runtimes(
    entries: dict, pagination: int, max_workers: int | None = None
) -> None:
    """
    ...
    for (log_id, _), fetched_runtime in zip(entries_to_update, results, strict=False):
        if fetched_runtime:
...
def extract_user_diary(
    ...
    while True:
        ...
        if table:
            ...
            for row in rows:
                ...
                cols = dict(zip(headers, row.find_all("td"), strict=False))
                ...
                react_div = (
                    production_col.find("div", {"class": "react-component"})
                    if production_col
                    else None
                )
                ...
                rating = (
                    int(rating["class"][-1].split("-")[-1]) / 2.0 if is_rating else None
                )
                # like column
                ...
                runtime = actions.get("data-film-run-time") or actions.get(
                    "data-film-runtime"
                )
    ...
    if not fetch_runtime and any(
        entry["runtime"] is None for entry in ret["entries"].values()
    ):
        warnings.warn(
...
def extract_user_wrapped(
    ...
    def retrieve_diary() -> dict:
        ...
        try:
            diary = extract_user_diary(
                username, year, fetch_runtime=fetch_runtime, max_workers=max_workers
            )
        except Exception as e:
            raise ValueError(
                f"Failed to retrieve diary for user {username}: {e}"
            ) from e
    ...
    def update_counters(
        date_info: str, day_counter: dict, month_counter: dict
    ) -> tuple:
    ...
    def update_milestones(
        entry_no: int, log_id: str, data: dict, milestones: dict
    ) -> dict:
    ...
    def update_watched_entries(
        log_id: str, data: dict, first_watched: dict, last_watched: dict
    ) -> tuple:
    ...
    for log_id, data in diary["entries"].items():
        ...
        if watched_date.year == year:
            ...
            total_runtime, total_review = process_entry(
                data, total_runtime, total_review
            )
            milestones = update_milestones(no, log_id, data, milestones)
            first_watched, last_watched = update_watched_entries(
                log_id, data, first_watched, last_watched
            )
